...
const ProductPage: React.FC = () => {
  ...
  const [nextPage, setNextPage] = useState<string | null>(
    `${process.env.NEXT_PUBLIC_BASE_URL}/api/products/?page=1`
  );
  ...
  const handlePageChange = (pageNumber: number) => {
    const url = `${process.env.NEXT_PUBLIC_BASE_URL}/api/products/?page=${pageNumber}`;
    ...
    <main>
      ...
      <div className="w-full flex justify-center space-x-2 my-4 ">
        <button
          ...
          disabled={!prevPage}
          className="px-2 sm:px-4 py-1 sm:py-2 bg-blue-500 text-white rounded disabled:opacity-50 hover:bg-blue-700 transition duration-200 ease-in-out"
        >
          Previous
        </button>
        {pageNumbers.map((number) => (
          <button
            ...
            className={`px-2 sm:px-4 py-1 sm:py-2 rounded hidden sm:inline-block ${
              currentPage === number
